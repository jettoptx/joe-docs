...
```
JettChat / Workspace UI
    ↓
Hermes OPTX API (:8643)
    ├── /api/sessions/{id}/chat/stream  → SSE streaming tokens
    ├── /api/sessions/new/chat/stream   → create + stream in one call
    ...
    ├── /api/tasks                      → SpacetimeDB DAG orchestration
    ├── /api/wallet                     → Tempo billing + escrow
    ├── /api/gateway-status             → capability probing
