...
description: JOULE templates, astro-encoded gaze keys, and the biometric proof submission flow.
icon: ScanFace
---

Each gaze verification session produces a **JOULE template** — a proprietary encoding of the user's AGT tensor sequence. The AARON protocol hashes this into a **astro_knot invariant** and stores it on-chain.

## JOULE Template

```typescript
// JOULE Template — astro-encoded gaze key
interface JOULETemplate {
  ...
  astroEncoding: string;        // proprietary encoding
  astroKnot: string;            // astro_knot invariant identifier
...
pub struct BiometricProof {
    ...
    pub id: u64,
    pub user_id: u64,
    pub astro_encoding: String,        // proprietary encoding
    pub astro_knot: String,            // astro_knot invariant identifier
