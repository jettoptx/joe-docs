...
**AARON** (Asynchronous Audit RAG Optical Node) is the biometric proof protocol that underpins the **Astro Knots** registry. It generates privacy-preserving gaze proofs by encoding AGT tensor sequences into astro_knot invariants, then anchoring them on Solana via CSTB attestations.
...
- **Astro-Encoded Proofs** — Agentive Gaze Tensor (AGT) sequences encoded as astro_knot invariants for privacy-preserving on-chain attestation.
