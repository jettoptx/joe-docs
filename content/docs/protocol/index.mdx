...
|------|-----------|-------------|
| **JETT Auth** | Joule Encryption Temporal Template Auth | Biometric gaze signature + SSO system on Jetson. The identity layer. |
...
- **N** — **Node**: Runs on a physical edge node (Jetson Orin Nano). Private compute that never exposes raw biometric data to the network.
...
| **JOE** | The Agent | JETT Optical Engine. Autonomous AI agent on the edge node. Executes tools, manages services, talks via Matrix chat. |
...
- **Privacy First** — Raw biometric data never leaves the edge node. Only opaque 32-byte proofs go on-chain.
