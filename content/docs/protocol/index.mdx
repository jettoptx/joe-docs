...
**AARON** (Asynchronous Audit RAG Optical Node) is the biometric proof protocol that underpins the **Astro Knots** registry. It generates quantum-resistant gaze proofs by encoding AGT tensor sequences into knot invariants, then anchoring them on Solana via CSTB attestations.
...
- **O** — **Optical**: Gaze biometrics via iris tracking. The camera becomes the key. MediaPipe FaceLandmarker captures COG/EMO/ENV tensor regions.
...
| **HEDGEHOG** | The Brain | MCP context bridge. Routes AI queries to Grok 4.1, stores gaze data in SpacetimeDB, tracks API usage. The memory layer. |
...
- **Quantum Resistant** — Agentive Gaze Tensor (AGT) sequences encoded as knot invariants — resistant to quantum attacks.
