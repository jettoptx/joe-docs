...
- **Body links cleaned up**: dropped the `text-decoration: underline` on `.prose a:hover` (no more underline-on-hover for body links) and the small globe SVG icon that `MdxLink` injected after every internal `/docs/...` reference. Body links still open the MOA overlay when their target maps to a node, but the affordance is now color-only (orange → lighter orange on hover).
